...
$sql = "SELECT advertises.*, users.*, adv_img.* FROM adv_img LEFT JOIN advertises ON advertises.id = adv_img.adv_id LEFT JOIN users ON users.id = advertises.user_id  WHERE description LIKE '$search' OR location LIKE '$search' OR title LIKE '$search' OR size LIKE '%$search' OR room LIKE '$search' OR bath LIKE '$search' ORDER BY created_at DESC LIMIT $start_from,$limit";
    ...
    <div class="container-fluid mt-5 ">
        <div class="row">
            ...
            if(empty($allAdv) == false){
                ...
                <div class="alert alert-info text-danger col-lg-10 m-auto text-center">لا توجد نتائج لعملية البحث!</div>
                ...
                if ($count > $limit){
                    for ($i=1; $i<=$page_count; $i++) {
                        if($i==$pn){
                            ...
                            $links .= "<a class='active btn btn-success ml-1' href='".$this->route->baseUrl()."Advertise/searchResult?page=".$i."'>".$i."</a>";
                        }
                        else {

                            $links .= "<a class='btn btn-primary btn-sm ml-1' href='".$this->route->baseUrl()."Advertise/searchResult?page=".$i."'>".$i."</a>";
                        }
                    }
                    if ($pn > $page_count) { ?>
...
<?php } else { ?>
    <div class="alert alert-info text-danger col-lg-10 mt-5 mx-auto text-center">لا توجد نتائج لعملية البحث!</div>
