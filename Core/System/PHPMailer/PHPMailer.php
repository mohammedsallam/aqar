...
class PHPMailer
{
    ...
    /**
     * Email priority.
     * Options: null (default), 1 = High, 2 = Normal, 5 = low.
     ...
     * as defined in RFC5322 section 2.6.4 or it will be ignored.
     *
     ...
     * Default of 5 minutes (300sec) is from RFC2821 section 4.5.2.2.
     *
     * @var int
     */
    public $Timeout = 300;
     ...
     * ```
     *
     * Alternatively, you can pass in an instance of a PSR-2 compatible logger, though only `debug`
        ...
        switch ($patternselect) {
            ...
            case 'pcre8':
                ...
                return (bool) preg_match(
                    '/^(?!(?>(?1)"?(?>\\\[ -~]|[^"])"?(?1)){255,})(?!(?>(?1)"?(?>\\\[ -~]|[^"])"?(?1)){65,}@)' .
                    '((?>(?>(?>((?>(?>(?>\x0D\x0A)?[\t ])+|(?>[\t ]*\x0D\x0A)?[\t ]+)?)(\((?>(?2)' .
                    '(?>[\x01-\x08\x0B\x0C\x0E-\'*-\[\]-\x7F]|\\\[\x00-\x7F]|(?2)))*(?2)\)))+(?2))|(?2))?)' .
        ...
        if (ini_get('mail.add_x_header') == 1
            ...
            and ((version_compare(PHP_VERSION, '7.0.0', '>=')
                ...
                or (version_compare(PHP_VERSION, '7.1.0', '>=')
                    and version_compare(PHP_VERSION, '7.1.2', '<')))
        ) {
            trigger_error(
                'Your version of PHP is affected by a bug that may result in corrupted messages.' .
                ' To fix it, switch to sending using SMTP, disable the mail.add_x_header option in' .
                ' your php.ini, switch to MacOS or Linux, or upgrade your PHP to version 7.0.17+ or 7.1.2+.',
                E_USER_WARNING
            );
        }
    ...
    public function createHeader()
    {
        ...
        // Only allow a custom message ID if it conforms to RFC 5322 section 2.6.4
     ...
     * Uses the 'relaxed' algorithm from RFC6376 section 2.4.2.
     ...
     * Uses the 'simple' algorithm from RFC6376 section 2.4.2.
